...
public class ScaryMaze extends JPanel implements Runnable, MouseMotionListener {
	...
	public void mouseMoved(MouseEvent e) {
		...
		int backroundColor = -11922292;
		//8. If the mouse falls off the path (if it is on the background)
	if(mouseColor == backroundColor){scare();}
				//call the scare method
		
		//13. If the mouse is on the end color
	int finishColor = -256 ;	
	if(mouseColor ==  finishColor) {JOptionPane.showMessageDialog(null, "Great job you did it..dang it ");}
				// pop up a message to tell them they won
	
	AudioClip sound = JApplet.newAudioClip(getClass().getResource("windowsXP.mp3"));
		sound.play();
	}
	...
	private void scare() {
		...
		AudioClip sound = JApplet.newAudioClip(getClass().getResource("loudNoise.wav"));
